...
class impronta_api {
     ...
     *
     * Si el endpoint cambia o deja de resolver, todas las llamadas fallan como
     * servicio no disponible; mantenerlo constante evita que el apiToken pueda
     * redirigirse a un destino no autorizado.
     *
    ...
    public static function subject(int $userid = 0): string {
        ...
        // User ID > 0 llega del token firmado: es el camino de la app, donde no
     ...
     * vídeo en el primer segmento marcado durante una incidencia anterior. La
     ...
     */
    public static function remember_session(
        string $path,
        int $userid,
        string $sessionid,
        string $playbackid = ''
    ...
    private static function has_curl_cli(): bool {
        ...
        if (!$tiene) {
            debugging('filter_impronta: curl(1) no disponible; la analitica se envia en modo sincrono', DEBUG_DEVELOPER);
        }
        return $tiene;
    }

    /**
     * Sends an analytics batch to Impronta.
     *
     * @param array $payload Analytics payload.
     * @return bool Whether the batch was accepted.
     */
    public static function post_events(array $payload): bool {
        ...
        if ($apikey === '') {
            debugging('filter_impronta: no apikey configured; dropping analytics batch', DEBUG_DEVELOPER);
            return false;
        }
        ...
        if (PHP_OS_FAMILY === 'Windows' || !function_exists('popen') || !self::has_curl_cli()) {
            ...
            if ($httpcode < 200 || $httpcode >= 300) {
                ...
                debugging("filter_impronta: POST /events devolvio HTTP {$httpcode}", DEBUG_DEVELOPER);
                return false;
            }
            return true;
        }
