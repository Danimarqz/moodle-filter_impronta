...
class cloudfront {
    ...
    public static function fetch(string $url, int $retries = 2) {
        ...
        while ($attempt < $retries) {
            ...
            // Curl_close() is a no-op since PHP 8.0; let the handle be released naturally.
            ...
            $safeurl = strtok($url, '?');
            debugging("filter_impronta: fetch {$attempt}/{$retries} failed for [{$safeurl}]. "
                . "HTTP {$http}. curl [{$err}] {$errmsg}", DEBUG_DEVELOPER);
        }
