...
class token {
    ...
    public static function generate(
        ...
        // -el navegador móvil no envía cookie-, así que is_mobile_app()
        ...
        // durante una incidencia anterior. Un token sin playbackid —los ya emitidos—
    ...
    /**
     * Checks the signed context for an authenticated renewal.
     *
     * This does not treat the context as playback authorization.
     */
    public static function signed_context(
        string $filename,
        string $token,
        int $expires,
        int $courseid,
        ...
        return hash_equals(self::generate(
            $filename,
            $expires,
            $courseid,
            $ip,
    ...
    public static function authorize(
        ...
        if ($mode === 'scorm') {
            if (
                $authorizationgroupid === '' || $playbackid === ''
                    || !self::validate(
                        $path,
                        $token,
                        $expires,
