...
class player {
    ...
    public static function render(string $filename, array $options = []): string {
        ...
        // colgado a los pocos minutos, en bucle. Con un id por render,
        ...
        if (!empty($options['token']) && !empty($options['expires'])) {
            ...
            // Is mobile app se marca DENTRO de la firma: es lo que permite que
            // playlist.php exija sesión de navegador sin cortarle el vídeo a la
            // app, que nunca manda la cookie: el
...
HTML;
        ...
        $setupconfig = $isaudio ? ['audioOnlyMode' => true]
            ...
            // y pre-play salta el layout de la página.
        ...
        $playlistjson = json_encode(
            $playlisturl,
            JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE
            | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_QUOT | JSON_HEX_APOS
        );
        $trackshtml = self::tracks_html(
            $filename,
            $options,
            $token,
            $expires,
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Pasamos el elemento, no el id: nombres nuevos como SERGAS1.1 contienen un punto
  // y videojs(id) los trata como selectores CSS invalidos.
  // Sin esta comprobacion el src()
  ...
  var player = videojs(document.getElementById('{$escapedid}'));
...
HTML;
     ...
     * @return string
     */
    private static function app_marker(
        string $filename,
        string $playlisturl,
        string $embedurl,
        string $token,
        ...
        $data = [
            ...
            'events' => $isaudio ? '' : token::endpoint_url(
                'events.php',
                $filename,
                $token,
                $expires,
            ...
            'session' => $isaudio ? '' : token::endpoint_url(
                'heartbeat.php',
                $filename,
                $token,
                $expires,
    ...
    private static function extras(
        ...
        $cfg = [
            ...
            'eventsUrl' => token::endpoint_url(
                ...
                $playbackid,
                $mode
            ), 'subject' => impronta_api::subject($userid), 'videoPath' => $filename, 'playlistUrl' => $playlisturl,
